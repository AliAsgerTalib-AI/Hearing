...
import { Phone, Volume2, Check, X, RotateCcw } from 'lucide-react';
import { ReceptionistScenario, ReceptionistTrial } from '../../lib/scenarios/ReceptionistScenario';
import { SoundCoinBadge } from './SoundCoinBadge';
import { useScenario } from '../../contexts/ScenarioContext';
import { calculateCoinReward } from '../../types/scenarios';

interface ReceptionistSessionProps {
...
export function ReceptionistSession({ audioContext, onClose, difficulty }: ReceptionistSessionProps) {
  const { awardCoins } = useScenario();
  const [phase, setPhase] = useState<SessionPhase>('intro');
  const [scenario] = useState(() => new ReceptionistScenario(audioContext, null as any, null as any));
  const [currentTrial, setCurrentTrial] = useState<ReceptionistTrial | null>(null);
  ...
  const startTrial = async () => {
    ...
    // Note: In a real implementation, this would use actual audio synthesis
    // For now, we're simulating the delay
    setTimeout(() => {
      setPhase('answering');
    }, 3000);
  };

  const handleAnswerSelect = (name: string) => {
    if (!currentTrial) return;

    const correct = scenario.checkAnswer(currentTrial, name);
  ...
  const nextOrComplete = () => {
    if (trialsCount < 5) {
      ...
      awardCoins(coins, `Complete Receptionist (Difficulty ${difficulty})`, 'receptionist');
      setPhase('complete');
    }
  };
        ...
        <div className="bg-gradient-to-r from-rose-500 to-pink-600 text-white p-6">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-3">
              <Phone size={24} />
              <h2 className="text-xl font-bold">Receptionist Game</h2>
            </div>
            <SoundCoinBadge balance={0} size="sm" />
          </div>
          <p className="text-sm opacity-90">Listen and identify the caller's name</p>
        </div>
          ...
          <AnimatePresence mode="wait">
              ...
              <motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <div className="space-y-4">
                  <p className="text-slate-700">
                    You'll hear a phone call from a receptionist. Listen carefully and identify the caller's name
                  ...
                  <div className="bg-teal-50 border border-teal-200 rounded-lg p-4">
                    <p className="text-sm text-teal-900">
                      💡 Tip: Focus on the center audio source. Background noise will vary based on difficulty.
                    </p>
                  </div>
                </div>
                <button
                  onClick={startTrial}
                  className="mt-8 w-full bg-gradient-to-r from-rose-500 to-pink-600 text-white font-semibold py-3 rounded-lg hover:shadow-lg transition-shadow"
                >
                  Start Trial
                </button>
              </motion.div>
            ...
            {phase === 'playing' && (
              <motion.div key="playing" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center gap-4">
                <motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 1, repeat: Infinity }}>
                  <Volume2 size={48} className="text-rose-500" />
                </motion.div>
                <p className="text-slate-700 font-medium">Listening...</p>
            ...
            {phase === 'answering' && currentTrial && (
              <motion.div key="answering" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <p className="text-slate-700 font-medium mb-4">Who called?</p>
                <div className="grid grid-cols-2 gap-3">
                  {currentTrial.options.map(name => (
                    <button
                      key={name}
                      onClick={() => handleAnswerSelect(name)}
                      className="p-3 rounded-lg border-2 border-slate-300 hover:border-rose-500 hover:bg-rose-50 transition-colors font-medium text-slate-700 hover:text-rose-600"
                    >
                      {name}
                    </button>
                  ))}
              ...
              <motion.div key="feedback" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center gap-4">
                {isCorrect ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-slate-600">The correct answer was: {currentTrial?.callerName}</p>
                  </>
                )}
                <button
            ...
            {phase === 'complete' && (
              <motion.div key="complete" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col items-center justify-center gap-4">
                ...
                <p className="text-2xl font-bold text-slate-900">Great Job!</p>
                ...
                <button
                  onClick={onClose}
                  className="mt-6 w-full px-6 py-3 bg-gradient-to-r from-rose-500 to-pink-600 text-white rounded-lg hover:shadow-lg transition-shadow font-semibold"
                >
                  Back to Hub
                </button>
              </motion.div>
